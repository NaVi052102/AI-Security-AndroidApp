...
class BiometricsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        var lastAppOpen = "System UI"

        LiveLogger.logData.observe(viewLifecycleOwner) { logText ->
            // Intercept app-switch logs from the SecurityEnforcer
            // Example Log: "📱 Switched to Facebook"
            if (logText.contains("Switched to")) {
                val newApp = logText.substringAfter("Switched to").trim()

                // Shift the current app to the "Previous" slot
                tvPreviousApp.text = lastAppOpen

                // Put the newly opened app in the "Current" slot
                tvCurrentApp.text = newApp
                lastAppOpen = newApp
            }
        }
        ...
        btnReset.setOnClickListener {
            viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
                ...
                withContext(Dispatchers.Main) {
                    ...
                    tvPreviousApp.text = "System UI"
                    tvCurrentApp.text = "Monitoring..."
                    lastAppOpen = "System UI"
