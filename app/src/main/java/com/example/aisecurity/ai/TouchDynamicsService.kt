...
class TouchDynamicsService : AccessibilityService() {
    ...
    // 🚨 ADDED BACK: This fixes your Unresolved Reference error
        ...
        firebaseDb.collection("Users").document(myUid).addSnapshotListener { snapshot, e ->
            ...
            fun checkMirrorState(stateField: String, targetSetting: String, physicalState: Boolean) {
                ...
                if (firebaseState != null && firebaseState != physicalState) {
                    ...
                    // 🚨 Increased cooldown to 45 seconds to prevent looping if the hack misses
    ...
    // 🚨 EMERGENCY COMMS COMBO HACK (FIXED DOUBLE TAP)

    // Drops shade ONCE, taps everything that is OFF exactly one time.
    ...
    private fun executeEmergencyCommsCombo() {
        ...
        serviceScope.launch(Dispatchers.IO) {
            ...
            // If everything is already on, exit silently.
            ...
            withContext(Dispatchers.Main) {
                ...
                try {
                    ...
                    // 2. FORCE DROP THE SHADE

                    val metrics = resources.displayMetrics

                    val swipePath = Path().apply {

                        moveTo(metrics.widthPixels * 0.85f, 1f) // Top right corner for Vivo/Xiaomi
                    ...
                    delay(800) // Wait for shade to drop



                    // 3. BUILD THE SURGICAL HIT LIST (Only target what is off)
                    ...
                    // 4. SURGICAL COMBO SCAN AND TAP

                    spatialSurgicalComboTap(rootInActiveWindow, targets)



                    // 5. SWIPE TO PAGE 2 AND TAP REMAINING
    ...
    // 🚨 SURGICAL TAP: Guarantees it only taps ONE item per category to prevent double-toggling
    ...
    private fun spatialSurgicalComboTap(rootNode: AccessibilityNodeInfo?, targets: MutableMap<String, List<String>>) {
        ...
        // Keep track of what we successfully tapped so we don't tap it again
        ...
        while (queue.isNotEmpty()) {
            ...
            // Check if this node matches any of our remaining targets
            ...
            for ((category, keywords) in targets) {
                ...
                if (keywords.any { k -> text.contains(k) || desc.contains(k) || text == k }) {
                    ...
                    successfullyTapped.add(category) // Cross it off the list immediately
                    ...
                    break // Break inner loop, move to next node
        ...
        // Remove successfully tapped categories from the main list so page 2 doesn't trigger them
    ...
    // Original Single Toggle UI Hack
        ...
        serviceScope.launch(Dispatchers.IO) {
            ...
            withContext(Dispatchers.Main) {
                ...
                try {
                    ...
                    toggled = targets.isEmpty() // If map is empty, it means we tapped it
